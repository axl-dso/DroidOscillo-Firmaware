...
int scope_main(void)
{
  ...
  while (1)
  {
    ...
    if ((htrigger_pos_stat != HTRIGGER_POS_LEFT_OF_DISPLAY) || (MCV->RunningMode == RUNMODE_FREE) || (MCV->Timescale <= 5))
    { // 80Mspsの時はサンプル飛び対策のため必ず待つ
      // バッファがたまるのを待つ
      //2ndLLIが読み込まれるまで待機		★時間のかかる処理
      while (!(LPC_GPDMA->INTTCSTAT & 0x1))
        CHECK_STATUS_UPDATE; // DMA Ch0のターミナルカウント割り込みフラグ監視
    }
    ...
    if (MCV->RunningMode == RUNMODE_NORMAL || MCV->RunningMode == RUNMODE_AUTO_FREE || MCV->RunningMode == RUNMODE_AUTO_NORMAL)
    {
      ...
      if (MCV->RunningMode == RUNMODE_AUTO_FREE)
      { // AUTO FREE

        while (1)
        {
          // waiting trigger	★時間のかかる処理
          if (LPC_ADCHS->INTS[1].STATUS & trigger_edge_reg_mask)
          {                                         // トリガーしたかどうか
          ...
          if ((LPC_GPDMA->C0LLI != (uint32_t)&auto_free_lli) && (MCV->BufferIsPending == FALSE))
          { // 一画面分のサンプルが溜まったている状態でバッファが開きなら

            if (MCV->Timescale <= TIMESCALE_1US)
            { // 80Mspsの時はADCを停めないとコピー中にバッファが書き換えられる
              stop_adc();
            }
      ...
      { // third_lliか4th_lliが読み込まれている時		= head:dma_stop+1  tall:dma_stop
        // またはトリガーが画面外左側にあるとき
        i = dma_stop_i + 1;
        if (htrigger_pos_stat != HTRIGGER_POS_LEFT_OF_DISPLAY)
        {
      ...
      // 触发搜寻
      Bool
          lower; // 如果缓冲区[i-1]的值小于触发值，则为TRUE
      lower = (MCV->TriggerEdge == TRIGGER_EDGE_UP) ? FALSE : TRUE;

      while (1)
      {
        if (buffer.mem_HW[i] >= MCV->VTrigger)
        { // トリガーを上回る
          if ((lower == TRUE) && (MCV->TriggerEdge == TRIGGER_EDGE_UP))
          { // up cross detected
            break;
          }
          lower = FALSE;
        ...
        {
          if ((lower == FALSE) && (MCV->TriggerEdge == TRIGGER_EDGE_DOWN))
          { // down cross detected
            break;
          }
          lower = TRUE;
  ...
  loop_end:
    ...
    if (MCV->UpdateFlags || lli_update_flag)
    {
      ...
      if (MCV->RunningMode == RUNMODE_NORMAL || MCV->RunningMode == RUNMODE_AUTO_FREE || MCV->RunningMode == RUNMODE_AUTO_NORMAL)
      {
        ...
        if (MCV->RunningMode == RUNMODE_NORMAL || MCV->RunningMode == RUNMODE_AUTO_NORMAL)
        { // NORMAL MODE or AUTO_NORMAL
          if (trgRequiredLength_wd > 0)
          { // 水平トリガーが画面内、もしくは画面外右側
          ...
          if (MCV->Timescale <= 5)
          { // 80MSpsの時にLLIの切り替え地点でサンプル飛びが発生するので応急処置
    ...
    else if (MCV->OneShotMode == TRUE)
    { // SINGLEモードでトリガーした後の処理
