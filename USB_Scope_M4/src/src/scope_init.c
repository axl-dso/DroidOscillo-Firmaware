...
__attribute__((section(".isr_vector"))) volatile uint32_t pseudo_vectortable[69]; //ベクターテーブル配置用空メモリ
...
void start_adc(void)
{
  ...
  uint32_t i = 0;
  for (i = 0; i < 100; i++)
  {                       // 変換し始めのサンプルは波形が歪むので捨てる
    LPC_ADCHS->FLUSH = 1; // FIFO FLUSH
  }
  //LPC_GPDMA->CONFIG = 1;
  LPC_GPDMA->C0CONFIG |= GPDMA_DMACCxConfig_E; // start DMA
  if (LPC_ADCHS->DESCRIPTOR[0][0] & 0x3fff00)
    LPC_ADCHS->TRIGGER = 1; // Set Trigger if descriptor0 MATCH_VALUE>0
}
